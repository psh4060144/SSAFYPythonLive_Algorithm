# list의 가장 큰 특징: sequence. indexing이 가능.
# 따라서, list의 요소에 접근할 때는 웬만하면 index로 접근하기. index로 접근하는 습관을 들여야 한다.
...
# 정렬: 2개 이상의 자료를 특정 기준에 의해 작은 값부터 큰 값(오름차순:ascending), 혹은 그 반대의 순서(내림차순:descending)로 재배열하는 것.
# Bubble Sort, Counting Sort, Selection Sort, Quick Sort, Insertion Sort, Merge Sort 등.


# Bubble Sort
# 인접한 두 개의 원소를 비교하며(두 개의 원소로 된 bubble 안에서 비교) 자리를 계속 교환하는 방식.
# 첫 번째 원소부터 인접한 원소끼리 계속 자리를 교환하면서 맨 마지막 자리까지 이동.
# 한 단계가 끝나면 가장 큰 원소가 마지막 자리로 정렬.
# 교환하며 자리를 이동하는 모습이 물 위로 bubble이 올라오는 것처럼 보여 Bubble Sort라고 함.
# O(n^2)


def bubble_sort_ascending(a, N):          	 # a = 정렬할 List, N = 원소 수
    for i in range(N-1, 0, -1):              # N-1번 원소의 index부터 0번 원소의 index까지 거꾸로 내려가며 탐색.
        for j in range(i):	            	 # index i만큼 탐색. 그러니까, a[i]의 좌측 전체를 찾음.
            if a[j] > a[j+1]:                # 앞이 뒤보다 크면
                a[j], a[j+1] = a[j+1], a[j]  # 뒤와 앞의 자리를 바꾼다.
